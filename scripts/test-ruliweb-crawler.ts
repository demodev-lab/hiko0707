...
async function testRuliwebCrawler() {
  console.log(chalk.cyan('🔍 루리웹 크롤러 테스트 시작...\n'))
  ...
  try {
    ...
    const existingPost = await repository.findBySourceAndPostId('ruliweb', 'test-1234')
  ...
  try {
    ...
    const results = await manager.crawl('ruliweb')
    ...
    if (results.length > 0) {
      ...
      if (result.hotdeals.length > 0) {
        ...
        console.log(chalk.gray(`- 상태: ${firstDeal.status}`))
        ...
        const soldOutCount = result.hotdeals.filter(deal => deal.status === 'expired').length
        console.log(chalk.yellow(`\n💡 품절 상품: ${soldOutCount}개`))
      }
      ...
      if (result.hotdeals.length > 0) {
        const repository = new SupabaseHotDealRepository()
        const firstDeal = result.hotdeals[0]
        const savedPost = await repository.findBySourceAndPostId('ruliweb', firstDeal.source_id)
        if (savedPost) {
...
// Run the test
testRuliwebCrawler().catch(console.error)