...
async function testClienCrawler() {
  console.log(chalk.cyan('🔍 클리앙 크롤러 테스트 시작...\n'))
  ...
  try {
    ...
    const existingPost = await repository.findBySourceAndPostId('clien', 'test-1234')
  ...
  try {
    ...
    const results = await manager.crawl('clien')
    ...
    if (results.length > 0) {
      ...
      if (result.hotdeals.length > 0) {
        const repository = new SupabaseHotDealRepository()
        const firstDeal = result.hotdeals[0]
        const savedPost = await repository.findBySourceAndPostId('clien', firstDeal.source_id)
        if (savedPost) {
...
// Run the test
testClienCrawler().catch(console.error)