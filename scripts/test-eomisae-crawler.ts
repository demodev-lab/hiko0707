...
async function testEomisaeCrawler() {
  console.log(chalk.cyan('🔍 어미새 크롤러 테스트 시작...\n'))
  ...
  try {
    ...
    const existingPost = await repository.findBySourceAndPostId('eomisae', 'test-1234')
  ...
  try {
    ...
    const results = await manager.crawl('eomisae')
    ...
    if (results.length > 0) {
      ...
      if (result.hotdeals.length > 0) {
        const repository = new SupabaseHotDealRepository()
        const firstDeal = result.hotdeals[0]
        const savedPost = await repository.findBySourceAndPostId('eomisae', firstDeal.source_id)
        if (savedPost) {
          ...
          // Verify date parsing
          const originalDate = new Date(savedPost.created_at)
          console.log(chalk.gray(`- 날짜 파싱 검증: ${originalDate.getFullYear()}-${String(originalDate.getMonth() + 1).padStart(2, '0')}-${String(originalDate.getDate()).padStart(2, '0')}`))
        }
      }
    }
  } catch (error) {
...
// Run the test
testEomisaeCrawler().catch(console.error)