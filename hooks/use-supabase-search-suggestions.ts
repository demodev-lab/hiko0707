...
export function useSearchSuggestions(query: string, delay = 300) {
  ...
  const mockSuggestions: Record<string, SearchSuggestion[]> = {
    ...
    '커': [
      ...
      { id: '38', text: '커피캅지', category: '식품/영양', count: 423 },
    ],
    '화': [
...
// Recent searches management with Supabase integration
export function useRecentSearches() {
  ...
  return {
    recentSearches,
    isLoading,
    addRecentSearch: addRecentSearchMutation.mutate,
    removeRecentSearch: removeRecentSearchMutation.mutate,
    clearRecentSearches: clearRecentSearchesMutation.mutate,
    isAddingSearch: addRecentSearchMutation.isPending,
    isRemovingSearch: removeRecentSearchMutation.isPending,
    isClearingSearches: clearRecentSearchesMutation.isPending,
  }
}